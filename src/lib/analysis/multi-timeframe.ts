...
function determineSignalDirection(zScores: number[]): "long_spread" | "short_spread" | "neutral" {
    ...
    if (avgZScore > 1) return "short_spread" // Spread high, expect revert down
    if (avgZScore < -1) return "long_spread"  // Spread low, expect revert up
    return "neutral"
}
...
export function analyzeMultiTimeframeConfluence(
    ...
    const weightedOpportunity = analyses.reduce(
        (sum, a) => sum + a.result.opportunityScore * a.weight,
        0
    ) / totalWeight

    // Find best and worst timeframes
    const sortedByScore = [...analyses].sort((a, b) => b.result.opportunityScore - a.result.opportunityScore)
    ...
    if (hasGoodAlignment && zScoreAgreement > 0.7 && qualityAgreement > 0.6 && alignedCount >= 3) {
        confidence = "high"
    } else if (hasModerateAlignment && zScoreAgreement > 0.5 && qualityAgreement > 0.4 && alignedCount >= 2) {
        confidence = "medium"
    ...
    if (allScoresIdentical && analyses.length > 2) {
        console.warn(`[MTF] Suspicious: All ${analyses.length} intervals have identical opportunity score: ${analyses[0].result.opportunityScore}`)
    }
    ...
    const premiumCount = analyses.filter(a =>
    ...
    const confluenceScore = Math.max(0, Math.min(100, Math.round(
        baseOpportunity +
        alignmentBonus +
        agreementFactor +
        qualityBonus +
...
function buildConfluenceNotes(
    ...
    if (allScoresIdentical && analyses.length > 2) {
        notes.push("⚠️ Data quality issue: All intervals show identical scores. Try clearing cache or using different intervals.")
    }
    ...
    notes.push(`${confidenceEmoji[confidence as keyof typeof confidenceEmoji]} ${confidence.toUpperCase()} confidence: ${alignedCount}/${analyses.length} timeframes aligned`)

    // Direction note
    if (direction !== "neutral") {
        const action = direction === "long_spread" ? "LONG spread (buy primary, sell secondary)" : "SHORT spread (sell primary, buy secondary)"
    ...
    const best = analyses.reduce((max, a) => a.result.opportunityScore > max.result.opportunityScore ? a : max)
    if (best.result.opportunityScore > 60) {
        notes.push(`⭐ Strongest signal on ${best.interval} timeframe (${best.result.opportunityScore}%)`)
    }

    // Specific timeframe notes
    const premiumCount = analyses.filter(a => a.result.volatilitySpread.signalQuality === "premium").length
    if (premiumCount >= 2) {
 ...
 */
export function getConfluenceIntervalOptions(): { value: string; label: string; description: string; isNative: boolean }[] {
    return [
 ...
 */
export function getSuggestedTimeframes(style: "scalping" | "intraday" | "swing" | "ultra-scalp"): string[] {
    switch (style) {
        case "ultra-scalp":
