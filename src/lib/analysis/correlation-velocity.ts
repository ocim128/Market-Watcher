...
export function calculateCorrelationVelocity(
    ...
    if (rollingCorrelations.length < velocityLookback + 1) {
        const currentCorr =
            rollingCorrelations.length > 0
                ? rollingCorrelations[rollingCorrelations.length - 1]
                : 0
        return {
            currentCorrelation: currentCorr,
            previousCorrelation: currentCorr,
...
function determineCorrelationRegime(
    ...
    if (Math.abs(current) >= STRONG_THRESHOLD) return "stable_strong"
    if (Math.abs(current) <= WEAK_THRESHOLD) return "stable_weak"
