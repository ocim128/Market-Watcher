...
function _PairHistoryCard({ data }: PairHistoryCardProps) {
  const qualityEntries = Object.entries(data.signalQualityDistribution).sort((a, b) => b[1] - a[1])
        ...
        <div className="flex items-center justify-between">
          <CardTitle className="text-base">
            ...
            <span className="text-muted-foreground text-xs ml-2">
              vs {data.primarySymbol.replace('USDT', '')}
            </span>
          </CardTitle>
          <TrendBadge trend={data.opportunityTrend} />
        </div>
        <CardDescription>
          Tracked {data.totalOccurrences} times since{' '}
              ...
              {data.recentSignals.map((signal, i) => (
                <div
                  ...
                  className={cn(
                    ...
                    signal.opportunityScore >= 70
                      ? 'bg-emerald-400'
                      : signal.opportunityScore >= 40
                        ? 'bg-amber-400'
                        : 'bg-slate-400'
                  )}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-lg">Historical Tracking</CardTitle>
              <CardDescription>
                {stats?.totalSnapshots} snapshots across{' '}
                {stats?.dateRange.start
                  ? new Date(stats.dateRange.start).toLocaleDateString()
                  : 'N/A'}{' '}
                -{' '}
                {stats?.dateRange.end ? new Date(stats.dateRange.end).toLocaleDateString() : 'N/A'}
              </CardDescription>
            </div>
          </div>
