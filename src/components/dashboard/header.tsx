...
export function Header() {
  ...
  const { scan, isScanning, lastScanTime, setCurrentPrimaryPair } = useScan()
  ...
  const handleScanSettingsChange = useCallback(
    (newSettings: { interval: IntervalType; totalBars: number; primaryPair: string }) => {
      ...
      // Immediately sync primary pair to context so MTF panel uses the correct pair
              ...
              <div className="flex flex-col items-end">
                <div className="flex items-center gap-1.5 text-xs font-medium">
                  <span className="text-foreground">
                    {scanSettings.primaryPair.replace('USDT', '')}
                  </span>
