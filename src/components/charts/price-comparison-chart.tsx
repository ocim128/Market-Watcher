...
function normalizePrices(prices: number[]): number[] {
    ...
    if (first === 0) return prices.map(() => 0)
...
export function PriceComparisonChart({
    ...
    useEffect(() => {
        if (!primarySeriesRef.current || !secondarySeriesRef.current) return
        if (primaryPrices.length === 0 || secondaryPrices.length === 0) return
        ...
        const primaryData: LineData[] = normalizedPrimary.slice(-length).map((value, i) => ({
            time: (timestamps
                ? timestamps[i] / 1000
                : (now - (length - i) * hourMs) / 1000) as Time,
            value,
        }))
